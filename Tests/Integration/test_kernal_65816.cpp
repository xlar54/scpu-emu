...
TEST( integration_open_register_bank_forces_exact_interrupt_timing )
{
	// The "@$" freeze, pinned. With the SuperCPU register bank OPEN,
	// $E000-$FFFF is served from the KS image -- and CMD's KS image is not a
	// whole KERNAL. ROM $9F40-$9F50, which lands at $FF40-$FF50, is all zeros
	// in both the 1.4 and 2.04 images, and that range holds the IRQ entry at
	// $FF48. The only real IRQ entry in the whole 128K image lives in the KT
	// half, used when the bank is CLOSED.
	//
	// So an interrupt taken while the bank is open executes $00 = BRK, which
	// vectors back to $FF48 and loops forever with the stack filling. Real
	// hardware never arrives there: the interrupt is taken before the bank
	// opens. An interrupt cache allowed to go stale can defer one until after
	// it opens, and correct code then hangs.
	//
	// Both defences are required: sample the lines every cycle while the bank
	// is open, and tick the bus per instruction so that sampling actually
	// happens.
	Machine816 m;
	if ( !m.haveROMs ) { std::printf( "(skipped: no ROMs) " ); return; }

	// Pacing armed at turbo, where the cache would otherwise be coarsest.
	m.mem.setPacing( 1400000000ull, 20000000u );

	// Bank closed: the relaxed threshold applies and batching is allowed.
	m.mem.m_KernalShadowBase = 0xE000;
	CHECK( !m.mem.fineTicksRequired() );

	// Bank open: exact timing, both ways.
	m.mem.m_KernalShadowBase = 0x6000;
	CHECK( m.mem.fineTicksRequired() );

	// And the cache must actually re-sample every cycle in that state: one
	// tick of one cycle has to be enough to pick up a line that just changed.
	m.bus.m_IRQ = false;
	m.mem.tickFast( 1 );
	CHECK( !m.mem.irqFast() );
